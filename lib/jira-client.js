...
export async function jiraRequest(baseUrl, bearerToken, endpoint, options = {}, retries = MAX_RETRIES) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorBody.errorMessages && errorBody.errorMessages.length > 0) {
          errorMessage += `\n\nError details:\n${errorBody.errorMessages.map(msg => `  - ${msg}`).join('\n')}`;
        }
    ...
    // Some successful writes (e.g. POST /issueLink) return 200/201 with an empty
    // body. Calling response.json() on those throws "Unexpected end of JSON
    // input", which would surface a successful write as an error. Treat an
    // unparseable/empty body on a 2xx as a null result rather than a failure.
    let data;
    try {
    ...
    if (process.env.DEBUG === 'true') {
      console.error(`[RESPONSE] ${response.status} - ${data === null ? 'empty' : JSON.stringify(data).length + ' bytes'}`);
    }
